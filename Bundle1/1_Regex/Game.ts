...
export default class Game {
    ...
    endGame() {
        ...
        info.innerHTML = `<p><span class="trophy dance">🏆</span> Congratulations!!!</p>`;

        let table = document.createElement('table');
        table.classList.add('score');
        for (let i = 0; i < this.maxLevel-1; i++) {
            let sol = localStorage['level'+(i+1)] || '';
            table.innerHTML += `<tr><td>Level ${i+1}</td><td>${sol}</td></tr>`;
        }
        var data = window.atob('RmxhZyA9IEhMe1JlZ0V4cC1UeWMwMG4tOTEyMzR9')
        table.innerHTML += `<tr><td>Success</td><td>` + data + `</td></tr>`;
